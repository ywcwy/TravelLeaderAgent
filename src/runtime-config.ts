...
function extractionAdapter(value: string | undefined): "fake" | "openai" {
  const selected = value?.trim().toLowerCase() || "fake";
  if (selected !== "fake" && selected !== "openai") throw new RuntimeConfigError("TRAVEL_EXTRACTION_ADAPTER must be 'fake' or 'openai'.");
  return selected;
}
