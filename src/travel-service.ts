...
export class TravelService {
  ...
  renormalizeTripLocations(tripId: string, options: { contextual?: boolean } = {}): { tripId: string; proposals: number; tripItems: number; changed: number } {
    ...
    const renormalize = (table: "proposals" | "trip_items"): number => {
      const rows = this.db.connection.prepare(`SELECT id, location, origin, destination, location_canonical_id, origin_canonical_id, destination_canonical_id, city, region, country, macro_region, location_provenance, location_inference_evidence, location_resolver_version, kind, shape FROM ${table} WHERE trip_id = ?`).all(tripId) as Array<{ id: string; location: string | null; origin: string | null; destination: string | null; location_canonical_id: string | null; origin_canonical_id: string | null; destination_canonical_id: string | null; city: string | null; region: string | null; country: string | null; macro_region: string | null; location_provenance: string | null; location_inference_evidence: string | null; location_resolver_version: string | null; kind?: string; shape?: string }>;
