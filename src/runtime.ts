...
export class TravelLeaderRuntime {
  ...
  constructor(config: RuntimeConfig, poller?: RuntimePoller) {
    ...
    const service = new TravelService(this.database, config.systemAdministratorId);
    const handler = new LineWebhookHandler(service, { channelSecret: config.channelSecret, officialAccountUserId: config.officialAccountUserId });
